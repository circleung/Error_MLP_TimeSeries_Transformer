...
def load_lit_model(base_model, model_path):
    lit_model = TimeSeriesLightningDecoderModule(
        base_model=base_model, learning_rate=1e-3
    )
...
def autoregressive_predictions(model, test_dataloader, thresh_mae=0.1):
    ...
    while True:
        ...
        # print(f"True Values: {past_values[:,:,:7]}")
        pred = lit_model(past_values[:, :, :7])
        ...
        if mae > thresh_mae or n_predictions > 50:
            break
...
def plot_mae(mae_arr):
    import matplotlib.pyplot as plt

    plt.figure(figsize=(10, 5))
    plt.plot(mae_arr, marker="o")
    plt.title("MAE over Predictions")
...
if __name__ == "__main__":
    ...
    lit_model_path = "/media/8TB_hardisk/sangam/timeseries_forecasting/src/custom_transformers/outputs_decoder/lightning_logs/version_2/checkpoints/epoch=17-step=293796.ckpt"
